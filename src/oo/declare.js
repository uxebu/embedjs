...
define(['embed', 'feature!lang-mixin', 'feature!lang-is', 'feature!lang-object', 'feature!array', 'feature!oo-extend', 'feature!oo-delegate'], function(embed){

	embed.declare = function(/*String*/ className, /*Function|Function[]*/ superclass, /*Object*/ props){
		//	summary: 
		...
		if(embed.isArray(superclass)){
			mixins = superclass;
			superclass = mixins.shift();
		}
		// construct intermediate classes for mixins
		if(mixins){
			embed.forEach(mixins, function(m, i){
		...
		embed.extend(ctor, { declaredClass: className, _constructor: props.constructor/*, preamble: null*/ });
		// special help for IE
		ctor.prototype.constructor = ctor;
		// create named reference
		return embed.setObject(className, ctor); // Function
	};
	
	embed.mixin(embed.declare, {
		_delegate: function(base, mixin){
			var bp = (base || 0).prototype, mp = (mixin || 0).prototype, dd = embed.declare;
			// fresh constructor, fresh prototype
			var ctor = dd._makeCtor();
			// cache ancestry
			embed.mixin(ctor, { superclass: bp, mixin: mp, extend: dd._extend });
			// chain prototypes
			if(base){ ctor.prototype = embed._delegate(bp); }
			// add mixin and core
			embed.extend(ctor, dd._core, mp || 0, { _constructor: null, preamble: null });
			// special help for IE
			ctor.prototype.constructor = ctor;
		...
		_extend: function(props){
			var i, fn;
			for(i in props){ if(embed.isFunction(fn=props[i]) && !0[i]){fn.nom=i;fn.ctor=this;} }
			embed.extend(this, props);
		},
		_makeCtor: function(){
		...
		_core: { 
			_construct: function(args){
				...
				if(a[0]){ 
					...
					if(/*embed.isFunction*/((fn = a[0].preamble))){ 
						a = fn.apply(this, a) || a; 
					}
				} 
				// prototype preamble
			...
			inherited: function(name, args, newArgs){
				...
				//		hoping the use in conjuction with `embed.hitch`. Calling an inherited 
