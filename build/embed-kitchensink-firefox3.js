...
var src=_18[i].getAttribute("src");
if(!src){
continue;
}
var m=src.match(_19);
...
var cfg=_18[i].getAttribute("djConfig");
if(cfg){
var _1a=eval("({ "+cfg+" })");
for(var x in _1a){
dojo.config[x]=_1a[x];
...
return _1.classList.contains(_2);
};
...
_8.classList.add(_a[i]);
}
};
dojo.removeClass=function(_b,_c){
if(_c===undefined){
_b.className="";
}else{
var _d=_7(_c);
for(var i=0,l=_d.length;i<l;i++){
_b.classList.remove(_d[i]);
}
}
};
})();
